 ...
 */
export function isUserAllowed({ user, permissions }: { user?: User; permissions: RequiredPermissions }) {
  if (!user) {
    return false;
  }
  ...
  if (Array.isArray(permissions)) {
    if (typeof permissions[0] === "string") {
      return (permissions as string[]).some((permission) => user.permissions.includes(permission));
    }

    return permissions.every((permissionGroup) =>
      (permissionGroup as string[]).some((permission) => user.permissions.includes(permission)),
    );
  }
