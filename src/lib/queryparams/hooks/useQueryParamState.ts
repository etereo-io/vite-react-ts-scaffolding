...
export function useQueryParamState<T = string>(key: string, options: UseQueryParamStateOptions<T> = {}) {
  const { params, applyParams, removeParams } = useQueryParams();

  const finalOptions: UseQueryParamStateOptions<T> = { storePrefix: "app-", defaultValue: null as T, ...options };
  ...
  const resetFallback = useCallback(() => {
    if (finalOptions?.fallbackStore) {
      localStorage.setItem(`${finalOptions.storePrefix}${key}`, finalOptions.defaultValue as string);
    }
  }, [finalOptions.defaultValue, finalOptions?.fallbackStore, finalOptions.storePrefix, key]);

  const value =
    (finalOptions?.fallbackStore
      ? params.get(key) ?? localStorage.getItem(`${finalOptions.storePrefix}${key}`)
      : params.get(key)) ??
  ...
  useEffect(() => {
    if (finalOptions?.fallbackStore) {
      localStorage.setItem(`${finalOptions.storePrefix}${key}`, value as string);
    }
